...
- (void)setupToolbars
{
    ...
    indexLabel.font = [UIFont systemFontOfSize:18.f];
   ...
   // [saveButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    //[saveButton setImage:[UIImage imageNamed:@"home_guangchang_baocun"] forState:UIControlStateNormal];
   ...
   // saveButton.clipsToBounds = YES;
    ...
    saveButton.layer.cornerRadius = 5 * ScreenWidthScale;
    [saveButton addTarget:self action:@selector(saveImage) forControlEvents:UIControlEventTouchUpInside];
    _saveButton = saveButton;
    _saveButton.hidden = _hideSaveBtn;
...
- (void)saveImage
{
    ...
    if (status == PHAuthorizationStatusRestricted ||
        ...
        [HHAlert showAlertString:@"保存失败,请打开相册权限"];
        return ;
    }
    
    if (_indicatorView.superview) {
        [HHAlert showAlertString:@"正在保存图片,别着急~"];
        return;
    }
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo;
{
     ...
     */
    if (error) {
        [HHAlert showAlertString:@"保存失败"];
        NSLog(@"error -> %@",error.localizedFailureReason);
    } else {
        [HHAlert showAlertString:@"保存成功"];
    }
}
...
- (void)layoutSubviews
{
    ...
    _indexLabel.center = CGPointMake(self.bounds.size.width * 0.5, 32.5);
    _saveButton.frame = CGRectMake(self.bounds.size.width * 0.5 - 35, self.bounds.size.height - 88, 70, 33);
}
