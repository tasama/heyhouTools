...
/** 根文件夹 */
static NSString * const MJDownloadRootDir = @"DownloadMusic";
...
- (void)didCompleteWithError:(NSError *)error
{
  ...
  //  }
    if (self.state == MJDownloadStateCompleted) {
    [MJDownloadNoteCenter postNotificationName:MJDownloadFinishedNotification
                                            object:self.url
                                          userInfo:@{MJDownloadInfoKey : self}];
    !self.stateChangeBlock ? : self.stateChangeBlock(self.state, self.file, self.error);
    }
...
- (void)deleteFileInfo:(MJDownloadInfo *)info
{
...
{
    [self.downloadInfoArray removeAllObjects];
}
...
- (BOOL)checkDownloadIngWithUrl:(NSString *)url
{
    if (url == nil) {
        return NO;
    }
