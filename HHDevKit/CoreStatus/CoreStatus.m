...
+(void)beginNotiNetwork:(id<CoreStatusProtocol>)listener{
    ...
    if(status.isNoti){
        ...
        HHLogDebug(@"CoreStatus已经处于监听中，请检查其他页面是否关闭监听！");
    ...
    [[NSNotificationCenter defaultCenter] addObserver:status selector:@selector(coreNetWorkStatusChanged:) name:kHHReachabilityChangedNotification object:nil];
...
+(void)endNotiNetwork:(id<CoreStatusProtocol>)listener{
    ...
    if(!status.isNoti){
        ...
        HHLogDebug(@"CoreStatus监听已经被关闭"); return;
    }
    
    //解除监听
    [[NSNotificationCenter defaultCenter] removeObserver:status name:kHHReachabilityChangedNotification object:nil];
