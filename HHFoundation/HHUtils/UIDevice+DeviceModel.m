...
#define kKeychainService_HeyHou   @"com.heyhou.social2"
#define kKeychainAcount_Heyhou   @"deviceId_heyhou"
...
- (BOOL)isSimulator
{
    return [self.deviceModel isEqualToString:Device_Simulator] ? YES : NO;
}

+ (NSString *)deviceUUID {
    NSString *udidStr = [SAMKeychain passwordForService:kKeychainService_HeyHou account:kKeychainAcount_Heyhou];
    
    if (udidStr.length == 0||!udidStr) {
        NSString *deviceId = [NSUUID UUID].UUIDString;
        [SAMKeychain setPassword: deviceId forService:
         kKeychainService_HeyHou account:kKeychainAcount_Heyhou];
        udidStr = [SAMKeychain passwordForService:kKeychainService_HeyHou account:kKeychainAcount_Heyhou];
    }
    return udidStr;
}
@end
